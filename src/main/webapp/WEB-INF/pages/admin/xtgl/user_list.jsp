                        ...
                        <div class="layui-card-body ">
                            <form class="layui-form layui-col-space5">
                                ...
                                <div class="layui-inline layui-show-xs-block">
                                    <input type="text" name="username" id="username" placeholder="请输入用户名"  autocomplete="off" class="layui-input"></div>
                                <div class="layui-inline layui-show-xs-block">
...
<script>
    layui.use('table', function(){
        ...
        table.render({
          elem: '#testT'
          ,url:'../../manageros/user/findUserAjax'
          ,toolbar: '#toolbarDemo'
          ,title: '用户数据表'
          //,toolbar:false
          ,cols: [[
             /* {type: 'checkbox', fixed: 'left'}
            , */{field:'xh', title:'序号', width:'10%', fixed: 'left', unresize: true,align:'center',type:'numbers'}   //unresize: true禁止拖动  type:'numbers' 序号排序
            ,{field:'username', title:'用户名', width:'15%',align:'center'}
            ,{field:'nickname', title:'真实姓名', width:'15%',align:'center'}
            ,{field:'createTime', title:'创建时间', width:'15%',align:'center'}
            ,{field:'updateTime', title:'修改时间', width:'15%',align:'center'}
            ,{field:'roleName', title:'角色', width:'15%',align:'center',templet: function(d){
                var arr = new Array();
                for(var i=0;i<d.roleList.length;i++ ){
                    arr.push(d.roleList[i].roleName);
                }
                return arr.join(",");
             }}
     ...
     //根据条件查询表格数据重新加载
      ...
      $('#searchB').on('click',
        ...
        table.on('toolbar(test)', function(obj){
             if(obj.event === 'add'){
                 xadmin.open('添加用户','../../manageros/user/toAddUser','','',true); //${pageContext.request.contextPath }
                  setTimeout(function(){
      ...
      table.on('tool(test)', function(obj){
       ...
       if(obj.event === 'edit'){
          //打开编辑页面
          xadmin.open('编辑用户','../../manageros/user/toEditUser?id='+data.id,'','',true); //${pageContext.request.contextPath }
          setTimeout(function(){
              layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
              tips: 3
             });
