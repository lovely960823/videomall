...
<html class="x-admin-sm">
    ...
    <body>
        ...
        <form class="layui-form" id="courseForm">
        	...
        	<input name="pic"  type="hidden" value="${course.pic}"/>
                ...
                <div class="layui-tab-content" >
                	<div class="layui-tab-item layui-show">
                   		<div class="layui-form-item">
		                      	   ...
		                      	   <span id="selectCity">
			                            <select name="parentId" required lay-verify="required"  id="parentId" lay-search=""  required="">
			                                ...
			                                <c:forEach items="${xkList}" var="xk">
			                                    <option <c:if test="${course.parentId eq xk.id }">selected="selected"</c:if> value="${xk.id}">${xk.name}</option>
			                                </c:forEach>
			                            </select>
			                         </span>
		                      </div>
		                      ...
		                      <div class="layui-input-inline" >
		                          <input type="text" id="name" name="name" value="${course.name }" required lay-verify="required"  autocomplete="off" class="layui-input">
		                      </div>
		                      <div class="layui-form-mid layui-word-aux">
		                      </div>
	                    </div>
	                    <div class="layui-form-item">
		                      ...
		                      <div class="layui-input-inline" >
		                      	   <input type="text" id="difficulty" name="difficulty" value="${course.difficulty }" required lay-verify="required" autocomplete="off" class="layui-input">
		                      </div>
		                      <div class="layui-form-mid layui-word-aux">
		                          <!-- <span class="x-red">*</span>不能为空 -->
		                      </div>
		                      ...
		                      <div class="layui-input-inline" >
		                          <input type="text" id="duration" name="duration" value="${course.duration }" required lay-verify="required" autocomplete="off" class="layui-input">
		                      </div>
		                      <div class="layui-form-mid layui-word-aux">
		                      </div>
	                    </div>
	                    <div class="layui-form-item">
		                      ...
		                      <div class="layui-input-inline" >
		                      	   <input type="text" id="price" name="price" required lay-verify="required" value="${course.price }" autocomplete="off" class="layui-input">
		                      </div>
		                      <div class="layui-form-mid layui-word-aux">
		                          <!-- <span class="x-red">*</span>不能为空 -->
		                      </div>
		                      ...
		                      <div class="layui-input-inline" >
		                          <input type="text" id="discountPrice" name="discountPrice" value="${course.discountPrice }" autocomplete="off" class="layui-input">
		                      </div>
		                      <div class="layui-form-mid layui-word-aux">
		                      </div>
	                    </div>
                        <div class="layui-form-item">
		                      	   ...
		                      	   <span id="selecttId">
			                            <select name="tId" required lay-verify="required"  id="tId" lay-search=""  required="">
			                                ...
			                                <c:forEach items="${teacherList}" var="teacher">
			                                    <option <c:if test="${course.tId eq teacher.id }">selected="selected"</c:if> value="${teacher.id}">${teacher.name}</option>
			                                </c:forEach>
			                            </select>
			                         </span>
		                      </div>
                            ...
                            <div class="layui-input-inline">
                               <input type="file" name="imageFile"  class="layui-input"/>
                            </div>
                            <div class="layui-input-inline">
                                <button type="button" class="layui-btn layui-btn-xs demo-reload" onclick="ylPic('${course.pic}')">查看</button>
                            </div>
                        </div>
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <textarea name="content" required lay-verify="required" class="layui-textarea" required="" id="editor"
                                 placeholder="课程介绍">${course.content}</textarea>
                            </div>
                        </div>
                    ...
                    <div class="layui-tab-item">
                        <table class="layui-table" style="width: 90%;margin: 0 auto;text-align: center;" >
						  ...
						  <tbody id="jscs">
						  	<c:forEach items="${courseDetailList }" var="detail">
						  		<tr>
						          <td><input type="text" name="dname"  autocomplete="off" class="layui-input" value="${detail.dname }"></td>
						          <td>
						          	<textarea name="dcontent" required lay-verify="required" class="layui-textarea" placeholder="简介">${detail.dcontent }</textarea>
						          </td>
						          <td>
						          	<input type="file" name="videoFile"  class="layui-input"/>
						          </td>
						          <td>
						          	<button type="button" class="layui-btn layui-btn-xs demo-reload" onclick="ylVideo('${detail.dvideo}')">预览</button>
						          	<button type="button" class="layui-btn layui-btn-danger" onclick="delJscs_update(this,'${detail.id}')">删除</button>
						          	<div id="${detail.id}">
							          	<input name="detailId"  type="hidden" value="${detail.id}"/>
							  			<input name="dvideo"  type="hidden" value="${detail.dvideo}"/>
						  			</div>
						         </td>
						      </tr>
						  	</c:forEach>
        ...
        layui.use(['form', 'element', 'upload', 'layer'], function() {
           ...
           //监听提交
            ...
            function(data) {
                ...
                $.ajax({
                    url:"${pageContext.request.contextPath }/manageros/course/update",
                    data:formData,
                    type:"POST",
         ...
         function delJscs_update(obj,id){
             $(obj).parent().parent().remove();
             $("#"+id).remove();
          }
         function addJscs(){
